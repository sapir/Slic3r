...
sub _G0_G1 {
    ...
    if (defined $point) {
        if (defined $z) {
            return $self->_G0_G1_pz($use_G0, $point, $z, $e, $F, $comment);
        } else {
            return $self->_G0_G1_p($use_G0, $point, $e, $F, $comment);
        }

    } else {
        if (defined $z) {
            return $self->_G0_G1_z($use_G0, $z, $e, $F, $comment);
        } else {
            return $self->_G0_G1_($use_G0, $e, $F, $comment);
        }
    }
}
...
sub extrude_loop {
    ...
    if ($self->config->spiral_vase) {
        ...
        if ($self->config->seam_position eq 'nearest') {
            ...
            my $obj = $self->layer->object;
            if (defined $self->layer && defined $self->get_seam_position($obj)) {
                $last_pos = $self->get_seam_position($obj);
            }
            my $point = $last_pos->nearest_point(\@candidates);
            $self->set_seam_position($obj, $point);
